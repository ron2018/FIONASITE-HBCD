...
anonymize () {
  ...
  # Ron Yang 09/2022 run python version of anonymizer for ABCD data.
...
# not going to use this in new ABCD
runSeriesInventions () {
  ...
  if [[ ! "$erg" == "" ]]; then
      ...
      SSDIR=${SDIR:4}
      /bin/python3 /var/www/html/server/bin/complianceCheck.py --suid=${SSDIR}
